<template>
	<view>
		...
		<view class="songSheet_wrap">
		  <!-- 控制遍历数量3个 -->
		  <view class="songSheet_item" v-for='(item, index) in newReleases' :key='index'>
			<!-- 图片容器 -->
			<view class="songSheet_img">
			  <!-- 图片 -->
			  <image mode="widthFix" :src="item.blurPicUrl" />
			</view>
			<!-- 标题 -->
			<view class="songSheet_titles">{{item.name}}</view>
			<view class="songSheet_titles">{{item.artist.name}}</view>
		  </view>
		</view>
	  </view>
